...
\begin{exercise}
  On note $H^1 = \{ v \in L^2(\Omega) \st \GRAD v \in L^2(\Omega) \}$
...
\begin{definition}[Consistance]
  ...
  solution exacte r\'eguli\`ere $u$ du probl\`eme associ\'e \`a~\eqref{eq:schema_1pas}, on a
  \begin{align}
  ...
  en temps et $q \in \N^*$ en espace si pour toute solution r\'eguli\`ere $u$ (du probl\`eme avant discr\'etisation)
...
\begin{remark}
  ...
  peut d\'ependre de la solution $u$ du probl\`eme associ\'e \`a~\eqref{eq:schema_1pas}.
\end{remark}
...
\begin{definition}[Convergence]
  ...
  \begin{align}
    ...
    | u(t_n , x_j) - u_j^n | = 0 ,
  \end{align}
  avec $Nh_t = T$ et $Mh_x = 1$ o\`u $u$ est la solution du probl\`eme associ\'e \`a~\eqref{eq:schema_1pas}.
...
\`a de l'information qui arrive et non \`a de l'information qui part. (!! Expliquer plus clairement cette phrase !!)
...
(en particulier stabilit\'e en norme $\infty$)
