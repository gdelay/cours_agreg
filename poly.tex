...
l'erreur commise par le sch\'ema en fonction de $h$ en utilisant une \'echelle logarithmique
pour les axes des abscisses et des ordonn\'ees (voir figure \ref{fig:ell_convergence}).
...
\begin{exercise}[Condition aux limites de Neumann]
  ...
  \item On ajoute une condition de moyenne nulle, le sch\'ema devient
    \begin{align*}
      ...
      \begin{array}{l}
        ...
        \dfrac{u_1 - u_0}{h} = \dfrac{u_M - u_{M-1}}{h} = 0 ,
        \\
        \dfrac{1}{h^2} \sum_{0 \leq j \leq M} u_j = 0 .
      \end{array}
      \right.
    \end{align*}
  ...
  \item Coder le sch\'ema propos\'e et faire une \'etude de convergence
    (tracer l'erreur du sch\'ema en fonction de $h$ en \'echelle logarithmique).
    Quel est l'ordre du sch\'ema ? ( !! quel $f$ ?? )
  \item Que se passe-t-il si on prend $f=1$ ? Commenter.
    !! Verifier ce qu'il se passe !!
  \end{enumerate}
\end{exercise}

!! Un dernier exercice pour finir la page ??
