...
Ce r\'esultat est \'enonc\'e dans la proposition suivante.
...
caract\'eristiques dans le sens inverse.

!! commentaires suppl\'ementaires sur la r\'eversibilit\'e en utilisant la figure !!

En suivant un raisonnement similaire, nous pouvons prouver d'autres r\'esultats 
laiss\'es ci-dessous en exercices.
...
\begin{exercise}[Conditions aux limites p\'eriodiques]
  ...
  \begin{equation}
    ...
    \right.
  \end{equation}
  avec $a\in \R$ et $u_0 \in C^1([0,1])$ qui v\'erifie la condition $u_0(0) = u_0(1)$,
  ...
  et $0 \leq f(x) < 1$.
  D'apr\`es vous, a-t-on besoin de la condition $u_0(0) = u_0(1)$?
  A-t-on besoin de la condition $u_0'(0) = u_0'(1)$?

  !! VERIFIER CE RESULTAT + HYPOTHESES !!
  -- Notamment ne faut-il pas compatibilit\'e des d\'eriv\'ees en $0$ et $1$ ?
\end{exercise}
...
\begin{exercise}[Prise en compte d'un terme source]
  ...
  Notez que l'on a pos\'e ce probl\`eme sur $\R$ entier pour ne pas avoir
...
exacte donn\'ee par \eqref{eq:sol_transport_per}.
...
Nous reportons les r\'esultats obtenus sur la figure \ref{fig:trans_per}.
...
se d\'egrade de plus en plus. La solution obtenue par le sch\'ema centr\'e va jusqu'\`a
diverger quand $N$, $M$ sont augment\'es.
...
\end{align}
o\`u $U_0$ est donn\'e par $(U_0)_j = u(0,x_j)$, $A$ est une matrice de $\R^{M \times M}$
et $F^n \in \R^M$ est un vecteur.
...
\begin{definition}[Stabilit\'e]
  ...
  $\forall (F^n)_{0 \leq n \leq N} \in \R^{N \times M}$, on a
  \begin{align}
...
\begin{definition}[Consistance]
  ...
  \begin{align}
    \lim_{\substack{N \to +\infty\\ M \to +\infty}} \max_{\substack{0\leq n \leq N\\ 0 \leq j \leq M}} 
  ...
  \begin{align}
    \forall N, M \geq 2 , \quad \max_{\substack{0\leq n \leq N\\ 0 \leq j \leq M}}
...
ais\'ees. La difficult\'e se trouve donc dans la preuve de la stabilit\'e.
Afin de faciliter ces preuves,
nous donnons maintenant des caract\'erisations de la stabilit\'e.
...
\begin{exercise}
  ...
  Soient $U_0 \in \R^{M}$ et $(F^n) \in \R^{N \times M}$.
  ...
  \begin{align}
    ...
    \forall 0 \leq n \leq N , \quad  U^n = A^n U_0 + h_t \sum_{\ell = 0}^{n-1} A^{n-\ell-1} F^{\ell} .
  \end{align}
  !! Cet exercice est-il vraiment utile ??
\end{exercise}
...
est que l'on a $u_j^1 = \sum_{\ell=0}^{M-1} a_{j \ell} u_{\ell}^{0}$.
En prenant $\calA_j(k) = \sum_{\ell=0}^{M-1} a_{j\ell} e^{2i\pi k (x_{\ell} - x_j)}$, on montre
...
Par la suite nous utiliserons la stabilit\'e de Von Neumann pour prouver qu'un sch\'ema
n'est pas stable.

\begin{remark}
  La stabilit\'e de Von Neumann correspond \`a la stabilit\'e du sch\'ema 
  dans le sens 
  \begin{align*}
    \| (u_j^n) \|_{*} 
    \leq C_1 \| U_0 \|_{\#} 
    + C_2 \| (F^n) \|_{*} ,
  \end{align*}
  avec des normes $\| \cdot \|_{*}$ et $\| \cdot \|_{\#}$ diff\'erentes de celles
  de la d\'efinition \ref{def:stabilite}.
\end{remark}

!! Harmoniser l'intervalle des j : 0 \`a M-1 ou 1 \`a M !!

!! Attention aux remarques sur les diff\'erentes normes :
on ne peut pas avoir l'\'equivalence entre les normes avec des constantes
qui ne d\'ependent pas des pas d'espace et de temps !!
!! remarque sur l'existence d'autres notions de stabilit\'e avec d'autres normes !!

!! pour la stabilit\'e au sens de Von Neumann : prouver la condition n\'ecessaire
+ simple remarque sur condition suffisante sous certaines conditions
dans une autre norme !! --- ne pas l'aborder ici !!
...
\begin{exercise}
  ...
  d'ordre 1 en temps et 1 en espace. 
\end{exercise}
...
!! grouper ces deux propositions ainsi que leur preuve !!
