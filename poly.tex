...
La force de compression \`a travers la barre est donn\'ee
par la loi d'\'elasticit\'e~\eqref{eq:def_flux}.
...
ces courbes peuvent \^etre plus complexes.
...
alors il faut connaitre $U^n$ et $U^{n-1}$ pour calculer $U^{n+1}$.
...
\begin{exercise}
  La figure~\ref{fig:ondes_c} a \'et\'e obtenue avec le sch\'ema~\eqref{eq:ondes_DF_exp}
  pour les donn\'ees $f=0$, $u_1=0$, $T=\frac25$, $N=40$, $M=50$, $c \in \{ \frac15, \frac12, 1 \}$
  et
  \begin{align*}
    u_0(x) =
    \left\{
    \begin{array}{l}
      x - \frac3{10} \text{ si } x \in (\frac3{10},\frac12) ,
      \\
      \frac7{10} - x \text{ si } x \in (\frac12, \frac7{10}) ,
      \\
      0 \text{ sinon} .
    \end{array}
    \right.
  ...
  \begin{enumerate}
  \item Coder le sch\'ema~\eqref{eq:ondes_DF_exp} et retrouver la figure~\ref{fig:ondes_c}.
  \item Observer que ce sch\'ema est instable si on ne respecte pas la condition de CFL:
    $c h_t \leq h_x$.
  \item Pour $c \in \{ \frac15 , \frac12 \}$, l'onde repr\'esent\'ee par la solution exacte
    n'a pas encore atteint les bords du domaine pour $T = \frac25$.
    Elle se comporte donc comme sur la droite r\'eelle $\R$.
    Calculer la solution exacte
    au temps final en utilisant la formule de d'Alembert (Th\'eor\`eme~\ref{thm:d_Alembert}).
    Comparer cette solution exacte avec la solution obtenue par le sch\'ema~\eqref{eq:ondes_DF_exp}.
    Observer que l'on converge vers la bonne solution lorsque $N,M \to +\infty$
    (sous r\'eserve que la condition de CFL soit respect\'ee).
    On pourra aussi retrouver num\'eriquement l'ordre du sch\'ema.
  \end{enumerate}
\end{exercise}
...
\begin{remark}
  ...
  Un sch\'ema \`a deux pas peut aussi \^etre utilis\'e pour les \'equations vues
  pr\'ec\'edemment (par exemple le sch\'ema saute-mouton pour l'\'equation des ondes,
  qui n'est pas abord\'e dans ce polycopi\'e).
  Comme pour une EDO d'ordre 2, on pourrait aussi r\'e\'ecrire l'\'equation des ondes comme
  un syst\`eme de deux EDP d'ordre 1. Si on discr\'etise ce syst\`eme, alors on obtient
  un sch\'ema \`a un pas de temps discr\'etisant l'\'equation des ondes.
\end{remark}

Sous forme matricielle, on peut \'ecrire le sch\'ema~\eqref{eq:ondes_DF_exp} comme
\begin{align*}
  U^{n+1}
...
en~\eqref{eq:mat_DF_Laplacien} (avec $h_x$ \`a la place de $h$),
$U^n$ le vecteur d'\'etat d\'efini
...
Pour essayer d'analyser les propri\'et\'es de ce sch\'ema,
on va l'\'ecrire sous une autre forme qui va permettre
...
Cependant, le grande difficult\'e est que la matrice $\hat{A}$
n'est pas sym\'etrique. On a alors seulement $\rho(\hat{A}) \leq \tn \hat{A} \tn_2$,
ce qui ne donne qu'une condition n\'ecessaire de stabilit\'e.
Une \'etude de la stabilit\'e de ce sch\'ema est propos\'ee dans le
chapitre X du livre
[Brigitte Lucquin: Equations aux d\'eriv\'ees partielles et leurs approximations].
